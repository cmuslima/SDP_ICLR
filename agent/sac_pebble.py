...
class SACAgent(Agent):
    ...
    def __init__(self, obs_dim, action_dim, action_range, device, critic_cfg,
        ...
        print('using original sac agent')
    ...
    def get_weight_norm(self, step):
        actor_weight_norms = []
        critic_weight_norms = []

        for param in self.actor.parameters():
            norm =utils.get_norm(param.data.cpu())
            actor_weight_norms.append(norm)
        for param in self.critic.parameters():
            norm =utils.get_norm(param.data.cpu())
            critic_weight_norms.append(norm)
        self.actor_weight_norm.update({step:actor_weight_norms})
        self.critic_weight_norm.update({step:critic_weight_norms})

        #print(critic_weight_norms, actor_weight_norms)
    @property
    def alpha(self):
        return self.log_alpha.exp()

    def get_policy(self, obs, sample=False):
        obs = torch.FloatTensor(obs).to(self.device)
        obs = obs.unsqueeze(0)
        dist = self.actor(obs)
        opt_action = dist.mean
        p_distribution = []
        for i in range(0, 100):
            action = dist.sample() 
            p_distribution.append(action)
        return p_distribution, opt_action
    ...
    def get_q_value(self, obs, action):
        ...
        return torch.min(q1.detach(), q2.detach()).item()
    ...
    def load(self, model_dir, step):
        print('self.device', self.device, 'step', step)
        if str(self.device) == 'cpu':
            print('self.device is cpu')
            self.actor.load_state_dict(
                torch.load('%s/actor_%s.pt' % (model_dir, step), map_location='cpu')
            )
            self.critic.load_state_dict(
                torch.load('%s/critic_%s.pt' % (model_dir, step), map_location='cpu')
            )
            self.critic_target.load_state_dict(
                torch.load('%s/critic_target_%s.pt' % (model_dir, step), map_location='cpu')
            )
        else:
            self.actor.load_state_dict(
