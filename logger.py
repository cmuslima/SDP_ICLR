...
COMMON_INTERACTION_FORMAT = [
    ('episode', 'E', 'int'),
    ('step', 'S', 'int'),
    ('target_r', 'R', 'float'),
    ('predicted_r', 'PR', 'float'), 
    ('target_q', 'Q', 'float'),
    ('predicted_q', 'PQ', 'float'),
    ('expert_action', 'NR', 'int'),
    ('duration', 'D', 'time'),
    ('total_duration', 'TD', 'time'),
]
...
class Logger(object):
    ...
    def _try_sw_log_histogram(self, key, histogram, step):
        #print('inside _try_sw_log_histogram')
        #print('key', key)
        #print('histogram', histogram)
        data_type = histogram.dtype
        #print('dtype of histogram', data_type)
        #print('step', step)
        if self._sw is not None:
    ...
    def log_param(self, key, param, step, log_frequency=None):
        ...
        #print('type param.weight.data', type(param.weight.data))
    ...
    def log_histogram(self, key, histogram, step, log_frequency=None):
        #print('inside log histogram')
        if not self._should_log(step, log_frequency):
            #print('inside this place where we return from the log histogram')
            return
        assert key.startswith('train') or key.startswith('eval')
        self._try_sw_log_histogram(key, histogram, step)

    def dump(self, step, save=True, ty=None):
        #print('am I dumping',save)
        if ty is None:
