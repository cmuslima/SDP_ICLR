...
class ReplayBuffer(object):
    ...
    def __init__(self, obs_shape, action_shape, capacity, device, window=1):
        ...
        #self.rewards_var = np.empty((capacity, 1), dtype=np.float32)
    ...
    def add(self, obs, action, reward, next_obs, done, done_no_max):
        ...
        #np.copyto(self.rewards_var[self.idx], reward_var)
    ...
    def add_batch(self, obs, action, reward, reward_var, next_obs, done, done_no_max):
        ...
        if next_index >= self.capacity:
            ...
            #np.copyto(self.rewards_var[self.idx:self.capacity], reward_var[:maximum_index])
            ...
            if remain > 0:
                ...
                #np.copyto(self.rewards_var[0:remain], reward_var[maximum_index:])
            ...
            #np.copyto(self.rewards_var[self.idx:next_index], reward_var)
    ...
    def relabel_with_predictor(self, predictor):
        ...
        for index in range(total_iter):
            ...
            pred_reward, pred_reward_var = predictor.r_hat_batch(inputs)
            self.rewards[index*batch_size:last_index] = pred_reward
            #self.rewards_var[index*batch_size:last_index] = pred_reward_var
    ...
    def sample(self, batch_size):
        ...
        #rewards_var = torch.as_tensor(self.rewards_var[idxs], device=self.device)
