...
class ReplayBuffer(object):
    ...
    def copy_over_buffer(self, buffer, reward_model):
        self.capacity = buffer.capacity
        self.device = buffer.device



        self.obses = buffer.obses
        self.next_obses = buffer.next_obses
        self.actions = buffer.actions
        self.extrinsic_rewards = buffer.rewards
        self.intrinsic_rewards = np.empty((self.capacity, 1), dtype=np.float32)
        self.not_dones = buffer.not_dones
        self.not_dones_no_max = buffer.not_dones_no_max
        self.window = buffer.window

        self.idx = buffer.idx
        self.last_save = buffer.last_save
        self.full = buffer.full      

        for i in range(0, self.capacity):
            obs = self.obses[i]
            action = self.actions[i]
            _, reward_hat_std = reward_model.r_hat_std(np.concatenate([obs, action], axis=-1))
            np.copyto(self.intrinsic_rewards[i], reward_hat_std)
