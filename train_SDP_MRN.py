        ...
        while self.step < self.cfg.num_train_steps:
            ...
            if self.step == (self.cfg.num_seed_steps + self.cfg.num_unsup_steps):
                ...
                if self.cfg.num_unsup_steps > 0:
                    ...
                    # relabel buffer
                    self.replay_buffer.relabel_with_predictor(self.reward_model,True, self.cfg.prior_data_amount)
                
                else:
                    print('Used the unsupervised exploration as sub-optimal data, not getting anymore unsupervised exploration')
                    # relabel buffer
                    self.replay_buffer.relabel_with_predictor(self.reward_model,True, self.cfg.prior_data_amount)
...
@hydra.main(config_path='config/train_sdp_MRN.yaml', strict=True)
def main(cfg):
    entity_name = ''
    workspace = Workspace(cfg)
    wandb.init(project="sdp", config=utils.flatten_dict(dict(cfg)), name=f'sdp_mrn_{cfg.env}_{cfg.max_feedback}_{cfg.seed}',
