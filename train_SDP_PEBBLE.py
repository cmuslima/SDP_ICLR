...
class Workspace(object):
    def __init__(self, cfg):
        ...
        # make env
        if 'METAWORLD' in cfg.env_domain and 'metaworld' in cfg.env:
            self.env = utils.make_metaworld_env(cfg)
            self.log_success = True
        elif 'DMCONTROL' in cfg.env_domain:
            self.env = utils.make_env(cfg)
        elif 'GYM' in cfg.env_domain:
            self.env = utils.make_gym_env(cfg)
        ...
        self.reward_model = RewardModel(
            ...
            train_batch_size = self.cfg.rm_train_batch_size)
    ...
    def learn_reward(self, first_flag=0):
        ...
        print('inside the start of learn reward')
        print('self.cfg.feed_type', self.cfg.feed_type)
        # get feedbacks
        ...
        print('self.labeled_feedback', self.labeled_feedback)
        train_acc = 0
        print('am I about to update my reward', self.cfg.reward_update)
    ...
    def run(self):
            ...
            if self.step == (self.cfg.num_seed_steps + self.cfg.num_unsup_steps):
                ...
                if self.total_feedback < self.cfg.max_feedback:
                    if interact_count == self.cfg.num_interact:
                        ...
                        if self.reward_model.reward_batch + self.total_feedback > self.cfg.max_feedback:
        ...
        utils.save_model(self.reward_model, self.work_dir, self.step)
        utils.save_model(self.agent,self.work_dir, self.step)
        utils.save_data('replay_buffer', self.replay_buffer)
        utils.save_data('reward_model', self.reward_model)
...
def main(cfg):
    ...
    entity_name = ''
    wandb.init(project="sdp", config=utils.flatten_dict(dict(cfg)), name=f'sdp_pebble_{cfg.env}_{cfg.max_feedback}_{cfg.seed}',
